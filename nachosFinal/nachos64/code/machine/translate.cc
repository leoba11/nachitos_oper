...
bool
Machine::ReadMem(int addr, int size, int *value)    //Lugar de donde leo, tamaño de lo que leo y lugar donde pongo lo leido
{
    int data;
...
//la vpn se calcula dividiendo la direccion virtual entre el tamaño de pagina (128)
//recibe la direccion virtual como parametro
	...
	//La busqueda del pageTable la hace si el vpn es mayor o igual al tamaño del pageTable o cuando la entrada vpn del pageTable es invalida
	entry = &pageTable[vpn];
    } else {
		...
		break;
			...
			//recorre todo el tlb, si la pagina de la posicion i es valida y ademas la pagina virtual o virtualPage es igual al vpn entonces se le asigna a la entrada lo que hay en
			//la posicion i del tlb
